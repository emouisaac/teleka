window.PlacesAutocomplete = window.PlacesAutocomplete || {
    POPULAR_TYPES: [
...
// A lightweight Places autocomplete module with Google + OSM fallback.
// Exposes global `PlacesAutocomplete` with init(pickupId, destinationId).
...
const PlacesAutocomplete = {
    ...
    async fetchSuggestions(query, sessionToken) {
        ...
        try {
            ...
            if (data.error_message || (data.status && data.status !== 'OK' && data.status !== 'ZERO_RESULTS')) {
                console.warn('Places API error:', data);
                if (recentPlaces.length) return recentPlaces;
                return [{ _error: true, message: `Places service error: ${data.error_message || data.status}. Check server logs and GOOGLE_MAPS_API_KEY.` }];
            }

            if (data.predictions && data.predictions.length) return [...recentPlaces, ...data.predictions];
...
// Initialize when DOM is ready if inputs exist
function _initPlacesIfNeeded() {
    const pa = window.PlacesAutocomplete;
    if (!pa) return;
    if (document.getElementById('pickup') && document.getElementById('destination')) {
        pa.init('pickup', 'destination');
    }
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _initPlacesIfNeeded);
} else {
    _initPlacesIfNeeded();
}